...
def testIntent():
    ...
    testLoki(inputLIST, ['ADVANCED'])
    print("")

    # s2fadvancedwhere
    print("[TEST] s2fadvancedwhere")
    inputLIST = ['張三應該在台北離開','張三在台北看到了馬莉','李四在台北不想教瑪莉','張三在台北遇到瑪莉很多次','老王在台北讀完文件後就丟掉了','李四在台北拍的照片被瑪莉看到了']
    testLoki(inputLIST, ['s2fadvancedwhere'])
    print("")

    # s2fadvancedwho
    print("[TEST] s2fadvancedwho")
    inputLIST = ['瑪麗最喜歡張三','瑪麗不喜歡王小明','瑪莉見過張三好幾次','李四不敢開王小明的刀','李四連這本書都不肯給張三','老王在會見之前就解雇了張三','李四從來不看批評王小明的文章','李四相信因為老闆罵了張三，每個員工都很生氣']
    testLoki(inputLIST, ['s2fadvancedwho'])
    print("")

    # s2fadvancedwhat
    print("[TEST] s2fadvancedwhat")
    inputLIST = ['馬莉買了書','那本書非常暢銷','每個人都會買帽子','這朵花馬莉最喜歡','馬莉一定會買帽子','李四特別喜歡那本書','那篇文章，李四從不看','李四讀完文件後就丟掉了','那份文件，他拿去丟掉了']
    testLoki(inputLIST, ['s2fadvancedwhat'])
    print("")
